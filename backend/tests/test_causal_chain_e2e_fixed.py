"""
Integration test for end-to-end causal chain propagation.

Tests complete causal chain tracking across complex topologies with multiple
failure scenarios. Validates that causal_chain arrays correctly track all
source devices causing downstream degraded/down status.
...
def test_full_causal_chain_fixed():
    client = TestClient(app)
    # Use full reset to avoid state leakage from other tests (shared sqlite file)
    reset_db()
    ...
    assert links, "expected link metrics after first tick"
    ...
    # Run another tick; engine must respect effective status and zero out metrics for devices and links
    if getattr(ENG, "engine", None) is not None:
        ENG.engine.run_tick()

    snap2 = get_v2_snapshot() or {}
    devs2 = snap2.get("devices", {})
    links2 = snap2.get("links", {})

    # Device zeroing: core_router must be 0 bps
    core2 = devs2.get("core_router", {})
    assert core2.get("bps", 0.0) == 0.0

    # Link zeroing: links adjacent to core_router should be zero
    # Specifically backbone_gateway<->core_router and core_router<->edge_router
    assert (
        links2.get("__".join(sorted(["backbone_gateway", "core_router"])), {}).get("bps", 0.0)
        == 0.0
    )
    assert links2.get("__".join(sorted(["core_router", "edge_router"])), {}).get("bps", 0.0) == 0.0
