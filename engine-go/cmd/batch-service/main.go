...
import (
	...
	"github.com/unoc/engine-go/internal/batch"
...
func main() {
	...
	logger := zerolog.New(os.Stdout).With().
		Timestamp().
		Str("service", "batch-service").
		Logger()

	// Get configuration from environment
	port := getEnv("BATCH_SERVICE_PORT", "50052")
	...
	logger.Info().
		...
		Msg("Starting Batch Operations Service")
	...
	batchSvc := batch.NewService(db, logger)
	pb.RegisterBatchServiceServer(grpcServer, batchSvc)

	// Register health check service
	healthSvc := health.NewServer()
	healthSvc.SetServingStatus("batch.BatchService", grpc_health_v1.HealthCheckResponse_SERVING)
	...
	go func() {
		...
		healthSvc.SetServingStatus("batch.BatchService", grpc_health_v1.HealthCheckResponse_NOT_SERVING)
	...
	logger.Info().
		...
		Msg("Batch Operations Service listening")
