...
const lossNum = computed(() => {
    const v = (device.value?.insertion_loss_db ?? 0) as number
    const n = Number(v)
    return Number.isFinite(n) ? n : 0
})
const lossText = computed(() => `${lossNum.value.toFixed(1)} dB`)
const lossColor = computed(() => (lossNum.value > 0 ? '#ef5350' : '#90caf9'))
