...
const emit = defineEmits<{ (e:'cancel'):void; (e:'submit', payload:{ name:string; max_down_mbps:number; max_up_mbps:number }):void }>()
...
const canSubmit = computed(()=> !nameError.value && !downError.value && !upError.value)

function submit(){
  if(!canSubmit.value) return
  emit('submit', { name: name.value.trim(), max_down_mbps: down.value!, max_up_mbps: up.value! })
}
</script>
<style scoped>
...
.form-grid input{ background:#111827; color:#e5e7eb; border:1px solid #374151; padding:.3rem .45rem; border-radius:4px; }
